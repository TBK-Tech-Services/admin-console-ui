...
export default function ManageExpensesPage() {
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return expensesData.filter((expense: Expense) => {
      ...
      if (filters.startDate) {
        ...
        if (new Date(expense.date) < startDate) return false;
      }
      if (filters.endDate) {
      ...
      if (filters.categoryId) {
        if (expense.categoryId !== parseInt(filters.categoryId)) return false;
      }
      if (filters.type) {
        if (expense.type !== filters.type) return false;
      }
      if (filters.villaId) {
        ...
        if (!isIndividualMatch && !isSplitMatch) return false;
      }
      return true;
    });
  }, [expensesData, filters]);

  const handleFiltersChange = (newFilters: ExpenseFilters) => setFilters(newFilters);
  const handleClearFilters = () => setFilters(initialExpenseFilters);
  ...
  const handleViewExpense = (expense: Expense) => { setSelectedViewExpense(expense); setIsViewModalOpen(true); };
  const handleEditExpense = (expense: Expense) => { setSelectedEditExpense(expense); setIsEditModalOpen(true); };
  const handleDeleteExpense = (expense: Expense) => { setSelectedDeleteExpense(expense); setIsDeleteModalOpen(true); };
  const handleCloseViewModal = () => { setIsViewModalOpen(false); setSelectedViewExpense(null); };
  const handleCloseEditModal = () => { setIsEditModalOpen(false); setSelectedEditExpense(null); };
  const handleCloseDeleteModal = () => { setIsDeleteModalOpen(false); setSelectedDeleteExpense(null); };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
      <ExpensesPageHeaderComponent onModalOpen={() => setIsAddModalOpen(true)} filters={filters} />
