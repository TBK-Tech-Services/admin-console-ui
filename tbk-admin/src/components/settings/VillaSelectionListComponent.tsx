...
export function VillaSelectionListComponent({villas, selectedVillas, onVillaToggle, disabledVillaIds = [], emptyMessage = "No villas available"}) {
    if (villas.length === 0) {
        return (
        ...
        {villas.map((villa) => {
            ...
            <div key={villa.id} className="flex items-center space-x-3">
                ...
                onCheckedChange={(checked) => 
                    onVillaToggle(villa.id.toString(), checked as boolean)
                }
                />
                <Label 
